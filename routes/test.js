...
var Answers = require('../models/answerSchema.js'); //db에 저장시키고 저장되어있는 것을 불러주는데 사용된다
...
router.get('/', function(req,res){
    ...
    BoardContents.count({deleted:false},function(err, totalCount){
       // db에서 날짜 순으로 데이터들을 가져옴
        ...
        //가장 인기있는 청원
        BoardContents.findOne({deleted:false}).sort({parti_cnt:-1}).exec(function(err, pageContent) {
            if(!err){
        ...
        //답변을 기다리는 청원
        BoardContents.find({$and:[{deleted:false},{parti_cnt:{"$gte":5}}]}).sort({start_date:-1}).limit(5).exec(function(err, pageContents) {
            if(!err){
        ...
        //청원목록
        BoardContents.find({deleted:false}).sort({start_date:-1}).skip(skipSize).limit(limitSize).exec(function(err, pageContents) {
            if(err) throw err;
            //board.ejs의 title변수엔 "Board"를, contents변수엔 db검색 결과 json데이터를 저장해 줌.

            console.log(hotPetition)
            console.log(uid)
            res.render('board', {
...
router.post('/',function(req,res){
    ...
    if(mode == 'add'){
        if(!req.session.uid){
            ...
            addBoard(sess,addNewTitle,addNewCategory,addNewContent,addNewPassword);
            res.redirect('/boards');
        }        
    }else{
...
router.get('/view',function(req,res){
    ...
    BoardContents.findOne({_id:contentId}, function(err, rawContent){
        ...
        rawContent.save(function(err){//변화된 조횟수 저장
            ...
            res.render('boardDetail',{content:rawContent,replyPage:reply_pg, uid:uid});//db에서 가져온 내용을 뷰로
        });
    })
});
...
router.post('/reply',function(req,res){
    ...
    if(!req.session.uid){
        ...
        var reply_id = req.body.replyId; //청원글 ID
      
        if(!reply_comment){
            reply_comment = "동의합니다.";
        }

        addComment(reply_id,sess,reply_comment);
...
router.get('/write',function(req,res){
    if(!req.session.uid){
        ...
        res.render('boardWrite',{uid:req.session.uid});
    }
});
...
router.get('/answer',function(req,res){
    ...
    Answers.count(function(err, totalCount){
       // db에서 날짜 순으로 데이터들을 가져옴
        ...
        pageNum = Math.ceil(totalCount/limitSize);
        Answers.find().sort({date:-1}).skip(skipSize).limit(limitSize).exec(function(err, pageContents) {
            if(err) throw err;
            //board.ejs의 title변수엔 "Board"를, contents변수엔 db검색 결과 json데이터를 저장해 줌.
            console.log(pageContents);
            res.render('boardAnswer', {contents: pageContents, pagination: pageNum,uid:uid});
        });
    });
});
...
function addBoard(session,title,category,content,password){
...
function addAnswer(session,petition,content){
    ...
    // newAnswers.petition.unshift({id:petition});
//    newAnswers.petition = petition;
    newAnswers.writer = session.email;
    newAnswers.contents = content;
    ...
    BoardContents.update({_id:petition}, {$set: {answer: newAnswers._id}}, function(err) {
        if(err) throw err;
    });
}